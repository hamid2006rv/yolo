...
def train():
    ...
    file_names = []
    with open(os.path.join(config.data_dir, 'voc_train.txt')) as f:
    ...
    with strategy.scope():
        ...
        model.load_weights(r"C:\yolo\YOLOv5-tf\weights\model_{}.h5".format(config.version), True)
    ...
    def train_fn():
        ...
        for step, inputs in enumerate(dataset):
            ...
            pb.add(1, [('loss', loss)])
            if step % steps == 0:
                model.save_weights(r"C:\yolo\YOLOv5-tf\weights\model_{}.h5".format(config.version))
...
def test():
    ...
    def test_fn():
        if not os.path.exists(r'C:\yolo\YOLOv5-tf\results'):
            os.makedirs(r'C:\yolo\YOLOv5-tf\results')
        file_names = []
        real_boxes = []
        real_labels = []
        with open(r'C:\yolo\YOLOv5-tf\voc_test.txt') as f:
            for line in f.readlines():
                line = line.replace('\n','')
                c = line.split(' ')
        ...
        for file_name in tqdm.tqdm(file_names):
            ...
            cv2.imwrite(f'C:\\yolo\\YOLOv5-tf\\results\\{name}', image)
            i+=1
...
def generate_tf_record():
    ...
    file_names = []
    with open(os.path.join(config.data_dir, 'voc_train.txt')) as reader:
