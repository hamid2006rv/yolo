...
def build_model(training=True):
    ...
    inputs = layers.Input([config.image_size, config.image_size, 1])
    ...
    x = csp(x, int(round(width * 256)), int(round(depth * 3)))
    x1 = x

    x = conv(x, int(round(width * 512)), 3, 2)
    x = csp(x, int(round(width * 512)), int(round(depth * 3)))
    x2 = x

    x = conv(x, int(round(width * 1024)), 3, 2)
    x = csp(x, int(round(width * 1024)), int(round(depth * 1)))
    ...
    p5 = layers.Conv2D(3 * (config.class_num + 5), 1, name=f'p5_{config.class_num}',
                       kernel_initializer=initializer, kernel_regularizer=l2)(x)

    x = conv(x, int(round(width * 512)), 1)
    x = layers.UpSampling2D()(x)
    x = conv(x, int(round(width * 512)), 3)
    x = layers.concatenate([x, x2])
    x = conv(x, int(round(width * 512)), 3)
    p4 = layers.Conv2D(3 * (config.class_num + 5), 1, name=f'p4_{config.class_num}',
                       kernel_initializer=initializer, kernel_regularizer=l2)(x)
    
    x = layers.UpSampling2D()(x)
    x = conv(x, int(round(width * 256)), 3)
    x = layers.concatenate([x, x1])
    x = conv(x, int(round(width * 256)), 3)

    p3 = layers.Conv2D(3 * (config.class_num + 5), 1, name=f'p3_{config.class_num}',
                       kernel_initializer=initializer, kernel_regularizer=l2)(x)

    
    x = layers.UpSampling2D()(x)
    x = conv(x, int(round(width * 128)), 3)
    x = layers.concatenate([x, x0])
    x = conv(x, int(round(width * 256)), 3)

    p2 = layers.Conv2D(3 * (config.class_num + 5), 1, name=f'p2_{config.class_num}',
                       kernel_initializer=initializer, kernel_regularizer=l2)(x)
    
    if training:
        return tf.keras.Model(inputs, [p5, p4, p3 , p2])
    else:
        return tf.keras.Model(inputs, Predict()([p5, p4, p3, p2]))
...
def process_layer(feature_map, anchors):
    ...
    feature_map = tf.reshape(feature_map, [-1, grid_size[0], grid_size[1], 3, 5 + config.class_num])

    box_centers, box_sizes, conf, prob = tf.split(feature_map, [2, 2, 1, config.class_num], axis=-1)
...
class ComputeLoss(object):
    ...
    def __call__(self, y_pred, y_true):
        loss = 0.
        anchor_group = [config.anchors[9:12], config.anchors[6:9], config.anchors[3:6], config.anchors[0:3]]
...
class Predict(layers.Layer):
    ...
    def call(self, inputs, **kwargs):
        y_pred = [(inputs[0], config.anchors[9:12]), 
                  (inputs[1], config.anchors[6:9]),
                  (inputs[2], config.anchors[3:6]),
                  (inputs[3], config.anchors[0:3])]
        ...
        for result in [process_layer(feature_map, anchors) for (feature_map, anchors) in y_pred]:
            ...
            prob = tf.reshape(prob, [-1, grid_size[0] * grid_size[1] * 3, config.class_num])
