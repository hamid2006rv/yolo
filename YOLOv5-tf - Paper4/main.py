...
def train():
    ...
    with strategy.scope():
        ...
        model.load_weights(r"C:\yolo\YOLOv5-tf - Paper4\weights\model_{}_{}.h5".format(config.version, 197382), True)

        model.summary()
        tf.keras.utils.plot_model(model,    
                                  to_file=r'C:\yolo\YOLOv5-tf - Paper4\model.png',
                                  show_shapes=True)
    ...
    def train_fn():
        ...
        for step, inputs in enumerate(dataset):
            if step % steps == 0:
                ...
                pb = tf.keras.utils.Progbar(steps, stateful_metrics=['loss'])
            step += 1
            image, y_true_1, y_true_2, y_true_3, y_true_4 = inputs
            
            y_true = (y_true_1, y_true_2, y_true_3, y_true_4)
            loss = distributed_train_step(image, y_true)
            pb.add(1, [('loss', loss)])
            if step % steps == 0:
                model.save_weights(r"C:\yolo\YOLOv5-tf - Paper4\weights\model_{}_{}.h5".format(config.version,step))
...
def test():
    ...
    def test_fn():
        if not os.path.exists(r'C:\yolo\YOLOv5-tf - Paper4\results'):
            os.makedirs(r'C:\yolo\YOLOv5-tf - Paper4\results')
        file_names = []
        real_boxes = []
        real_labels = []
        with open(r'C:\yolo\YOLOv5-tf - Paper4\val.txt') as f:
            for line in f.readlines():
                line = line.replace('\n','')
                c = line.split(' ')
        ...
        model.load_weights(r"C:\yolo\YOLOv5-tf - Paper4\weights\model_{}_{}.h5".format(config.version,155156), True)
        ...
        for file_name in tqdm.tqdm(file_names):
            ...
            cv2.imwrite(f'C:\\yolo\\YOLOv5-tf - Paper4\\results\\{name}', image)
            i+=1
...
def write_tf_record(queue, sentinel):
    ...
    while True:
        ...
        y_true_1, y_true_2, y_true_3, y_true_4 = util.process_box(boxes, label)
        ...
        features = tf.train.Features(feature={'in_image': byte_feature(in_image),
                                              ...
                                              'y_true_4': byte_feature(y_true_4)})
...
class AnchorGenerator:
    ...
    def get_boxes():
        boxes = []
        lines = open(r'C:\yolo\YOLOv5-tf - Paper4\train.txt').readlines()
        for line in lines:
            line = line.replace('\n','')
            f = line.split(' ')
            box,_ = util.load_label(f[1:])
